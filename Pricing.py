...
def Pricing():
    ...
    with upper_panel:
        ...
        result = final_df[["STOCK_CODE","DEPARTMENT","CATEGORY","STOCK_NAME","UNIT_PRICE_FOR_WEEK3","UNIT_PRICE_FOR_WEEK2","UNIT_PRICE_FOR_WEEK1","NEXT_WEEK_PREDICTION"]]
        fig = go.Figure(data=[go.Table(columnwidth=[1.4,2,1.5,2.4,1.8,1.8,1.8,2],header=dict(values=("<b>STOCK CODE<b>","<b>DEPARTMENT<b>","<b>CATEGORY<b>","<b>STOCK NAME<b>","<b>UNIT PRICE WEEK3<b>","<b>UNIT PRICE WEEK2<b>","<b>UNIT PRICE WEEK1<b>","<b>NEXT WEEK PREDICTION<b>"), fill_color='#00568D', font_color="#ffffff", align=['center'], line_color='#ffffff', font_size = 13,height=35),cells=dict(values=[result.STOCK_CODE,result.DEPARTMENT,result.CATEGORY,result.STOCK_NAME,result.UNIT_PRICE_FOR_WEEK3,result.UNIT_PRICE_FOR_WEEK2,result.UNIT_PRICE_FOR_WEEK1,result.NEXT_WEEK_PREDICTION],fill_color = [['white','lightgrey']*3200], align=['left'], font_size = 12))])
        ...
        color_scale = alt.Scale(
                    domain=['UNIT PRICE', 'PREDICTED UNIT PRICE'],  # Replace with your Price Type values
        ...
        fig = alt.Chart(pd_df_lsrr_pivot).mark_bar().encode(
            x=alt.X('STOCK_NAME:N', title='STOCK NAME', axis=alt.Axis(labelAngle=-80, labelFontSize=8, titleFontWeight='normal',titleFontSize=14)),  # Change font weight here
            y=alt.Y('Price:Q', title='PRICE', axis=alt.Axis(titleFontWeight='normal',titleFontSize=14)),  # Change font weight here
